...
export function registerWhatToEatTool(server, recipes) {
    server.tool("mcp_howtocook_whatToEat", "不知道吃什么？根据人数直接推荐适合的菜品组合", {
        ...
        // 获取所有荤菜
        let meatDishes = recipes.filter((recipe) => recipe.category === '荤菜' || recipe.category === '水产');
        // 获取其他可能的菜品（当做素菜）
        let vegetableDishes = recipes.filter((recipe) => recipe.category !== '荤菜' && recipe.category !== '水产' &&
        ...
        for (const meatType of meatTypes) {
            if (selectedMeatDishes.length >= remainingMeatCount)
                break;
