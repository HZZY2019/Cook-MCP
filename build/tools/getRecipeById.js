...
export function registerGetRecipeByIdTool(server, recipes) {
    server.tool("mcp_cook_getRecipeById", "根据菜谱名称或ID查询指定菜谱的完整详情，包括食材、步骤等", {
        ...
        // 如果还没有找到，尝试模糊匹配名称
        if (!foundRecipe) {
            foundRecipe = recipes.find(recipe => recipe.name.toLowerCase().includes(query.toLowerCase()));
        }
        // 如果仍然没有找到，返回所有可能的匹配项（最多5个）
        if (!foundRecipe) {
            const possibleMatches = recipes.filter(recipe => recipe.name.toLowerCase().includes(query.toLowerCase()) ||
                recipe.description.toLowerCase().includes(query.toLowerCase())).slice(0, 5);
